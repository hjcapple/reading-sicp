...
(define (reduct x y) (apply-generic 'reduct x y))
...
(define (install-scheme-number-package)
  ...
  (put 'reduct '(scheme-number scheme-number) (tag-2-args reduct-integers))
  (put 'greatest-common-divisor' '(scheme-number scheme-number) (tag-2-args gcd))
...
(define (install-polynomial-package)
  ...
  (define (put-op-notag name op)
    (put name '(polynomial polynomial)
         (lambda (x y) (op x y)))
    (put name '(polynomial scheme-number)
         (lambda (x y) (op x (number->poly y (variable x)))))
    (put name '(scheme-number polynomial)
         (lambda (x y) (op (number->poly x (variable y)) y))))
  (put-op 'add add-poly)
  (put-op 'mul mul-poly)
  (put-op 'sub sub-poly)
  (put-op 'div div-poly)
  (put-op 'rem rem-poly)
  (put-op-notag 'reduct reduct-poly)
  (put-op 'greatest-common-divisor gcd-poly)
...
;; b)
(define (gcd-numbers nums)
  (cond ((= (length nums) 1) (car nums))
        ((null? nums) 1)
        (else (gcd (car nums) (gcd-numbers (cdr nums))))))

(define (gcd-terms-b a b)
  (let ((L (gcd-terms-a a b)))
    (let ((coeff-list (map coeff L)))
      (let ((k (gcd-numbers coeff-list)))
        (map (lambda (t) (make-term (order t) (div (coeff t) k))) 
             L)))))
               
(define gcd-terms gcd-terms-b)

(define (gcd-poly p1 p2)
  (if (same-variable? (variable p1) (variable p2))
    (make-poly (variable p1)
               (gcd-terms (term-list p1) (term-list p2)))
    (gcd-poly p1 (poly->poly p2 (variable p1)))))
...
(define (reduct-poly p1 p2)
  (if (same-variable? (variable p1) (variable p2))
      (let ((result (reduce-terms (term-list p1) (term-list p2))))
        (list (make-poly (variable p1) (car result))
              (make-poly (variable p1) (cadr result))))
      (reduct-poly p1 (poly->poly p2 (variable p1)))))

(define (reduct-integers n d)
  (let ((g (gcd n d)))
    (list (/ n g) (/ d g))))
...
(module* main #f
  ...
  (print-poly "number" (number-rat (add rf1 rf2)))
  (print-poly "denom" (denom-rat (add rf1 rf2)))
